...
#include "../volumeHUD/DDCBrightnessPacket.h"
#include <assert.h>
#include <stdio.h>
#include <string.h>
...
int main(void) {
    ...
    checksum(reply);
    assert(VHDDCParseBrightness(reply, &current, &maximum));
    ...
    for (unsigned i = 0; i < sizeof(fields) / sizeof(fields[0]); i++) {
        ...
        invalid[fields[i]] ^= 1;
        checksum(invalid);
        assert(!VHDDCParseBrightness(invalid, &current, &maximum));
    }
    reply[10] ^= 1;
    assert(!VHDDCParseBrightness(reply, &current, &maximum));
    ...
    reply[9] = 0;
    checksum(reply);
    assert(VHDDCParseBrightness(reply, &current, &maximum) && current == 0 && maximum == 100);
    reply[9] = 100;
    checksum(reply);
    assert(VHDDCParseBrightness(reply, &current, &maximum) && current == maximum);
